...
(defn assemble-top-bar
  ...
  [:div {:id :top-bar}
        ...
        [:span {:id :top-bar--library-version :class :color--muted} (-> options :version)]
...
(defn assemble-page
  ...
  ; @return (string)
  [state options file-data]
  (html5 {} [:html (-> state (assemble-head options file-data))
                   (-> state (assemble-body options file-data) hiccup/unparse-class-vectors hiccup/unparse-css)]))
